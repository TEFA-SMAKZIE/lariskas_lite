...
class AuthService {
  ...
  Future<String> getDeviceId() async {
    try {
      if (Platform.isAndroid) {
        ...
        // Using androidId as unique identifier
        return androidInfo.id; // This is Android ID, unique per device
      } else if (Platform.isIOS) {
  ...
  Future<void> logout(context) async {
    try {
      final userProvider = Provider.of<UserProvider>(context, listen: false);
      final serialNumberId = userProvider.payload?.serialNumberId;
      final token = await storage.read(key: 'token');
      if (serialNumberId != null && token != null) {
        await http.put(
          Uri.parse(
              '${ApiConstants.baseUrl}/api/serial-number/$serialNumberId/logout-device-id'),
          headers: {
